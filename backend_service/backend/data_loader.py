...
class DataLoader:
    ...
    def load_user_data(self) -> Dict[str, Any]:
        """Load user data from MongoDB, or fallback to CSV/Sample and seed DB"""
        # 1. Try MongoDB
        if users_collection is not None:
            try:
                user_doc = users_collection.find_one({"user_id": self.user_id})
                if user_doc:
                    # Remove _id as it's not JSON serializable nicely in this flow usually
                    if "_id" in user_doc:
                        del user_doc["_id"]
                    return user_doc
        ...
        # 2. Fallback to existing logic (File/CSV/Sample)
        data = self._load_from_local_source()
        
        # 3. Seed MongoDB with this initial data
        if users_collection is not None:
            try:
                ...
                print(f"Seeded MongoDB for user {self.user_id}")
            except Exception as e:
                print(f"Error seeding MongoDB: {e}")
                
        return data

    def _load_from_local_source(self) -> Dict[str, Any]:
    ...
    def save_user_data(self, data: Dict[str, Any]):
        ...
        if users_collection is not None:
            try:
                ...
                print(f"Saved data to MongoDB for user {self.user_id}")
            except Exception as e:
                print(f"Error saving to MongoDB: {e}")
                
        # Also save to local file for backup/verification if needed (keeping existing behavior partly)
        try:
            base_dir = "/tmp/user_data" if os.environ.get("VERCEL") else "user_data"
